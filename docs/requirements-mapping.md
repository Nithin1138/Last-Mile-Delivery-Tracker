...
| **Idempotency Support** | [`backend/app/api/orders.py`](../backend/app/api/orders.py) | `POST /api/orders` | `tests/test_api.py::test_idempotent_order_creation` | `OrderCreatePage.tsx` |
| **Notification Provider Abstraction** | [`backend/app/services/notification_service.py`](../backend/app/services/notification_service.py) | Internal service | Verified in event logging | Console & Resend logs |
